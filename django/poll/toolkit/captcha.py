...
在Python程序中生成验证码并不算特别复杂，但需要三方库Pillow的支持（PIL的分支），因为要对验证码图片进行旋转、
扭曲、拉伸以及加入干扰信息来防范那些用OCR（光学文字识别）破解验证码的程序。
下面的代码封装了生成验证码图片的功能，大家可以直接用这些代码来生成图片验证码，不要“重复发明轮子”。

注意：代码在生成验证码图片时用到了三种字体文件，使用时需要添加字体文件到应用目录下的fonts目录中。
    ...
    def __init__(self, width, height, fonts=None, color=None):
        ...
        self._fonts = fonts if fonts else \
            [os.path.join(os.path.dirname(__file__), 'fonts', font)
             for font in ['ArialNI.ttf','Couri.ttf','Georgia.ttf']]
